...
- [x] Navigation overhaul (2026-07-12) — header nav relabelled & completed: Quotes / Fence calculator / Price list / Admin / accent "+ New quote" button (creates a blank editor quote via shared `lib/blankQuote.ts`; "Home" duplicate removed); mobile menu gains Price list + Admin + New quote; AdminLayout back link now "Back to app" → /quotes. Quotes list shows `quotes.title` as the job name (getJobName prefers title). Row delete now uses two-click ConfirmButton — previously a single accidental click deleted a quote instantly (three empty test drafts were lost to this during verification). All 5 nav items + 6 admin pages + edit/delete/new-quote buttons browser-verified.
