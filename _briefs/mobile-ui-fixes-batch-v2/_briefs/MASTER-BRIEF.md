...
2. Check the brief's dependencies. Read the brief's intro section for a `Depends on:` line. Parse it and verify each referenced brief's PR is merged on master.
   ...
   **Active dependency chains as of brief 025+ (current batch)**:
   - Briefs 019-024 are all merged on master — no current-batch dependencies among them
   - Brief 025 (property map sidebar) — depends on brief 023 merged ✅
   - Brief 026 (title bar job name) — depends on brief 019 merged ✅
   - Brief 027 (PWA icon fix) — depends on brief 023 merged ✅
   - All three briefs in this batch (025, 026, 027) are INDEPENDENT of each other — they touch different files and can all be PR'd in one Codex invocation
...
11. **Hard-coded stop points** (Codex must stop at these and report, even if more briefs are in the inbox):
    - **No hard stops apply for the current batch (briefs 025-027).** All three are independent and can be PR'd in one invocation. The natural stop is when `00-inbox/` is empty.
