...
function colorBondHeights(variables: Variables) {
  return String(variables.profile_code ?? "GZAG") === "GTRIM"
    ? [1800, 2100]
    : [1500, 1800, 2100];
}
...
export function InlineHeightEditor({
  ...
  if (productCode === "COLORBOND") {
    const options = colorBondHeights(variables);
    const value = options.includes(selectedHeight) ? selectedHeight : options[0];
    return (
      <select
